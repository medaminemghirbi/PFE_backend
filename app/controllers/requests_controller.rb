class RequestsController < ApplicationController
  ...
  def index
    ...
    render json: @requests, include: %i[mission freelancer]
  end
  ...
  def update
    @request = Request.find(params[:id])
		@mission = Mission.where('id = ?',@request.mission_id)
    unless @mission.freelancer_id.nil?
      if @request.update(post_params2)
        @requestfreelancer = Mission.where('id = ?',
                                           @request.mission_id).update(freelancer_id: @request.freelancer_id)
  ...
  def getrequestacceptedbyclient
    ...
    render json: @request, include: %i[mission freelancer]
  end
  ...
  def getrequestacceptedbyfreelancer
    ...
    @request = Request.where(mission_id: @mission.ids).where('status = ?',
                                                             status = 1).where(freelancer_id: params[:freelancer_id])
